import {Row, Form, Button, Modal, Image, FloatingLabel} from 'react-bootstrap';
...
const Create = ({ highlightmarket, nft}) => {
    ...
    const [mostsoldnegative, setmostsoldnegative] = useState(false)
    const handlemostsoldClose = () => setmostsoldnegative(false)
    const handlemostsoldShow = () => setmostsoldnegative(true)
    ...
    const createNFT = async () => {
        if(!image || !price || !name || !description || !mostsold || !demouri || !round) return
        try{
        ...
        await (await highlightmarket.makehighlight(nft.address, id, listingPrice, mostsold, getclassint(classification), getmapint(map), demouri, round, getplayerint(player))).wait()
        } catch (error) {
    ...
    const [duplicatedhighlight, setduplicatedhighlight] = useState(false)
    const handleduplicatedClose = () => setduplicatedhighlight(false)
    const handleduplicatedShow = () => setduplicatedhighlight(true)

    const loadboolens = async() => {
        let duplicatedhighlight = false
        duplicatedhighlight = await highlightmarket.appear_or_not(map.concat(demouri,String(round), player))
        setduplicatedhighlight(duplicatedhighlight)
        if(duplicatedhighlight)
        {setShow(false)}
    }
    ...
    const handleShow = () =>{
        loadboolens()
        if(!price || !name || !description || !mostsold || !demouri || !round) {
            handlenotfullShow()
            return}
        if(price <= 0){
            handlepriceShow()
        ...
        if(mostsold <1 || !isInteger(mostsold)){
            handlemostsoldShow()
            return
        }
        if(duplicatedhighlight){
            handleduplicatedShow()
            return
        }
                ...
                <main role='main' className='col-lg-12 mx-auto' style={{maxWidth: '1000px'}}>
                    <div className='content mx-auto'>
                        ...
                        >
                            ...
                            <FloatingLabel
                            controlId="floatingInput"
                            label="Maximum number of times it can be sold (should be positive)"
                            className="mb-3"
                        >
                            <Form.Control onChange={(e) => setmostsold(e.target.value)} type='number' placeholder='Maximum number of times it can be sold (should be positive)' />
                            </FloatingLabel>
                            ...
                            <div className='d-grid px-0'>
                                <Button onClick={handleShow} varient='primary'>
                                    Create NFT
                                </Button>
                                ...
                                <Modal show={mostsoldnegative} onHide={handlemostsoldClose}>
                                    <Modal.Header closeButton>
                                        <Modal.Title>Error</Modal.Title>
                                    </Modal.Header>
                                    <Modal.Body>
                                        <p2>"Maximum number of times it can be sold" should be an integer larger or equal to 1.</p2>
                                    </Modal.Body>
                                    <Modal.Footer>
                                    <Button variant="primary" onClick={handlemostsoldClose}>
                                        Close
                                    </Button>
                                    </Modal.Footer>
                                </Modal>
                                <Modal show={duplicatedhighlight} onHide={handleduplicatedClose}>
                                    <Modal.Header closeButton>
                                        <Modal.Title>Error</Modal.Title>
                                    </Modal.Header>
                                    <Modal.Body>
                                        <p2>The NFT of this highlight has been created.</p2>
                                    </Modal.Body>
                                    <Modal.Footer>
                                    <Button variant="primary" onClick={handleduplicatedClose}>
                                        Close
                                    </Button>
                                    </Modal.Footer>
                                </Modal>
